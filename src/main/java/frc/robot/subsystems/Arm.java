...
public class Arm extends SubsystemBase {
    ...
    public Arm(boolean isTuning) {
        ...
        TrapezoidProfileSubsystem(new TrapezoidProfile.Constraints(k_maxPivotVel, k_maxPivotAcc), k_pivotHomeRad, 0.02);
    ...
    protected void useState(TrapezoidProfile.State setpoint) {
        ...
        if(setpoint.position >= k_maxPivotAngleRad) {
            m_pivotPID.setReference(k_maxPivotAngleRad, CANSparkMax.ControlType.kPosition, k_PIVOT_SLOT_ID, m_ffEffort);
        } else if (setpoint.position <= k_pivotHomeRad) {
            m_pivotPID.setReference(k_pivotHomeRad, CANSparkMax.ControlType.kPosition, k_PIVOT_SLOT_ID, m_ffEffort);
        } else {
    ...
    public Command setPivotGoal(double pivotGoalRad) {
        System.out.println("Pivot Goal Set");
    ...
    public void calcFFEffortModified() {
        m_ffEffort = Math.cos(getPivotAngleRad() - k_pivotHorizontalRad) * calcPivotKG();
    }
    ...
    public double getPivotAngleRadModified() {
        return getPivotAngleRad() - k_pivotHorizontalRad;
    }
    ...
    public void tune() {
        ...
        m_pivotTuningSetpoint = Units.radiansToDegrees(k_pivotHomeRad);
        ...
        m_pivotKG_b_GainEntry = m_tab.add("Pivot Kg_b", m_pivotKG_b_Gain).withPosition(4, 0).getEntry();
        m_pivotKG_m_GainEntry = m_tab.add("Pivot Kg_m", m_pivotKG_m_Gain).withPosition(4, 1).getEntry();
        ...
        m_pivotTuningSetpointEntry = m_tab.add("Pivot Tuning Setpoint (deg)", m_pivotTuningSetpoint).withPosition(5, 0).withSize(2, 1).getEntry();
    ...
    public void tuningPeriodic() {
        ...
        m_pivotSetpointBuffer = m_pivotTuningSetpointEntry.getDouble(Units.radiansToDegrees(k_pivotHomeRad));

        if(m_pivotSetpointBuffer <= Units.radiansToDegrees(k_pivotHomeRad)) {
            var pivotTuningSetpoint = Units.radiansToDegrees(k_pivotHomeRad);
