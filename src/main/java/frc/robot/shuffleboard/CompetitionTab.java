...
public class CompetitionTab {
    ...
    public CompetitionTab(DriveBase driveBase, Claw claw, HashMap<String, Command> eventMap) {
        ...
        // m_autoChooser.setDefaultOption("Do Nothing", new doNothing(driveBase));
        // m_autoChooser.addOption("Do Nothing", new doNothing(m_driveBase));        
        // m_autoChooser.addOption("Cube and Taxi", new scoreCubeTaxi(driveBase));
        // m_competitionTab.add("Auto", m_autoChooser).withWidget(BuiltInWidgets.kComboBoxChooser).withPosition(7, 3).withSize(2, 1);
        
        m_competitionTab.add(m_usbCamera).withPosition(0, 0).withSize(7, 6);
    ...
    public Command getPlannedAuto() {
        pathGroup = PathPlanner.loadPathGroup(m_pathChooser.getSelected(), true, new PathConstraints(2, 1), new PathConstraints(2, 1), new PathConstraints(2, 1), new PathConstraints(2, 1));
